...
public class Joint extends Component {
    ...
    protected static Messenger myMessenger = Messenger.please(
            "Joint class", 2);
    ...
    public int getJointNumber() {return jointNumber;}
    ...
    public void addAnAction (Action a){actions.add(a);}

    //  protected int bestActionIndex= Integer.MAX_VALUE;
    private int myWakeCount= 0;
    public int getWakeCount(){return myWakeCount;}
    public void incWakeCount(){myWakeCount ++;}
    private int myUseCount= 0;
    public int getUseCount(){return myUseCount;}
    protected void incUseCount(){myUseCount ++;}

    //accessor methods
    public List<LinkCommand> getInputDrainCommands(){return inputDrainCommands;}
    public LinkCommand getInCmd(int i){return inputDrainCommands.get(i);}
    public Link getInLink(int i){return inputDrainCommands.get(i).getTarget();}
    public List<LinkCommand> getOutputFillCommands(){return outputFillCommands;}
    public LinkCommand getOutCmd(int i){return outputFillCommands.get(i);}
    public Link getOutLink(int i){return outputFillCommands.get(i).getTarget();}

    //peculiar that a join needs masterClear because it's supposed to be devoid of state
    ...
    @Override
    public String getTypeString() {return "Joint";}
            ...
            if(pp != null)
                {
                    ...
                    p= ("made Joint " + getName() + " of Type " + t
                            + " with parent " + pn);
                }
            myMessenger.line(p);
        }
    ...
    protected void addAnOutputLink(Link lk, int max)
        {
            if(getOutputFillCommands().size() > max)
                myMessenger.error("A " + getFullName() 
                        + " may have at most " + max + " data outputs");
            else addOutputLink(lk);
        }
    
    public void addAnOutputLink(Link lk){addOutputLink(lk);}
    ...
    protected void addAnInputLink(Link lk, int max)
        {
            if(getInputDrainCommands().size() > max)
                myMessenger.error("A " + getFullName() 
                        + " may have at most " + max + " data inputs");
            else addInputLink(lk);
        }
    
    public void addAnInputLink(Link lk){addInputLink(lk);}
    ...
    private class WakeUpCommand extends JointCommand{
        ...
        public String getMyType() { return ("WakeUpCommand"); }
    }// end of class WakeUp
    ...
    public void printMyTopology()
    {
            ...
            String s = name + " has " + plural(in, " input") + ", " +
                    plural(out, " output") + " and " + plural(a, " action");
            myMessenger.line(s);
            return;
        }// end of printMyTopology

    // to check for duplicate attachments
 /*   public boolean freeOfDuplicates()
       {
            return ok;
        } // end of hasDuplicateLinkCommand
*/
            ...
            for(LinkCommand C : inputDrainCommands)
                {
                    ...
                    if(t != this)
                        {
                            myMessenger.line(L.getName() + " output fails to point to "
                                    + this.getName());
                            ok= false;
                        }
                }
            for(LinkCommand C : outputFillCommands)
                {
                    ...
                    if(t != this)
                        {
                            myMessenger.line(L.getName() + " input fails to point to "
                                    + this.getName());
                            ok= false;
                        }
                }
            ...
            if(getInputDrainCommands().size() < minIn)
                {
                    ans= false;
                    myMessenger.error("A " + getFullName() 
                            + " must have at least " + minIn + " data inputs");
                }
            if(getOutputFillCommands().size() < minOut)
                {
                    ...
                    myMessenger.error("A " + getFullName() 
                            + " must have at least " + minOut + " data outputs");
                }
            return ans;
        }
///firing with useCoun 
    public void printMyStatistics()
        {
            myMessenger.line( getFullName() + " fired " + getUseCount()
                    + " times but woke " + getWakeCount() + " times");
    ...
    static public void printStatistics()
        {
            ...
            myMessenger.line("All Joints fired " + uses
                    + " times but woke " + wakeUps 
                    + " times.  Use to wakup ratio is " + ratio + "%");
       //     printDelays();
