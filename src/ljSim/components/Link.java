...
public class Link extends Component {
    ...
    public static int getLinkNumber() {return linkNumber;}
    ...
    public TimedValue getOutput(){return myOutputTimedValue;}
    public TimedValue getInput(){return myInputTimedValue;}
    ...
    protected Time getOutputTime(){return getOutput().getTime();}
    protected Time getInputTime(){return getInput().getTime();}
    ...
    public JointCommand getSinkCommand() {return mySinkCommand;} //belongs to a Joint
    public JointCommand getSourceCommand() {return mySourceCommand;} //belongs to a Joint
    
    public boolean inputFULL(){return myInputTimedValue.hasValue();}
    public boolean outputFULL(){return myOutputTimedValue.hasValue();}
  ...
  ///passing data  
    ...
    protected Link(String name, Component parent) {
        ...
        myMessenger.line("made and cleared Link " + getName() + " of Type " + t
                + " with parent " + getMyParent().getName());
        return;
    }
    ...
    @Override
    public String getTypeString(){return "Link";}
   ...
   @Override
    ...
    protected String computeState()
        {
            String ans  = null;
            if(inputFULL() && outputFULL())  ans= "FULL";
            if(!inputFULL() && !outputFULL())ans= "EMPTY";
            if(!inputFULL() && outputFULL())
                {// an impossible case - report error
            ...
            if( outputFirst()) ans= "FILLing";
            if( inputFirst()) ans= "DRAINing";
            if( ans == null)
                {// can't decide on direction
    ...
    public LinkHandle makeAhandle() {return new LinkHandle(this); }
    ...
    public void printMyTopology()
        {
            ...
            myMessenger.line(getName() + " gets input from "
                    + in.getName());
            myMessenger.line(getName() + " sends output to "
                    + out.getName());
        }// end of printMyTopology
    
    private String getStateString()
        {
            if(myInputTimedValue.getValue() != null )
                {//input has data
    ...
    {
        String s, ss, sss;
        s= getFullName() + " passed " + plural(passCount, " item") +
                " and " + plural(spaceCount, " space");
