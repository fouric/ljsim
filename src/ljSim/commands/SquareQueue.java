...
public class SquareQueue {
    ...
    public Command getNextCommand() {
      ...
      public void printPendingTasks()
          {
              myMessenger.line("There are " + commandsPending + " CommandsPending "
                      + " with maximum of " + maxCommandsPending);
      ...
      public void printStatistics()
          {
       //       String s= "Command did " + commandsDone + " maxPending= " + maxCommandsPending;
          }
/*
      public static void testMe()
          {
              Messenger m= Messenger.please("test");
              SquareQueue x= new SquareQueue(m);
              Command[] ca;
              ca= new Command[10];
              Command[] cb;
              cb= new Command[10];
              Joint j= OneInOneOutJoint.please("testJoint", null); 
              
              for(int i= 1; i<9 ; i++)
                  {
                      ca[i]= new SortedJointCommand (null);
              ...
              for(int i= 1; i<9 ; i++)
                  {
                      ca[i].enQueueMe(Time.please(i*1010));
                  }

              for(int i= 1; i<9 ; i++)
                  {
                      cb[i].enQueueMe(Time.please(i*440));
                  }
            }//end of testMe
 */         
